...
class FastRustDetector:
    ...
    def _extract_features_vectorized(
        ...
        metal_scores = (
            lc(f["metal_mask"], s, u, safe_mean, np.float32, 0) if "metal_mask" in f else np.ones(n_seg, dtype=np.float32)
        )
    ...
    def _compute_dynamic_gates(self, features_valid: np.ndarray) -> Dict[str, float]:
        ...
        # Defaults match your prior hardcoded guardrails
        dyn = {
            ...
            "v_dark": self._robust_percentile(v_vals, 25, 160.0),
        ...
        # clamp to sane ranges (avoid bizarre percentiles on tiny sets)
        dyn["a_warm"] = float(np.clip(dyn["a_warm"], 118.0, 150.0))
        dyn["b_warm"] = float(np.clip(dyn["b_warm"], 118.0, 150.0))
        dyn["a_red"] = float(np.clip(dyn["a_red"], dyn["a_warm"], 170.0))
        dyn["a_red_hi"] = float(np.clip(dyn["a_red_hi"], dyn["a_red"], 180.0))
        dyn["b_orange"] = float(np.clip(dyn["b_orange"], dyn["b_warm"], 175.0))
        dyn["b_orange_hi"] = float(np.clip(dyn["b_orange_hi"], dyn["b_orange"], 185.0))

        dyn["v_very_dark"] = float(np.clip(dyn["v_very_dark"], 25.0, 90.0))
        dyn["v_dark"] = float(np.clip(dyn["v_dark"], dyn["v_very_dark"] + 10.0, 200.0))
        dyn["v_hi"] = float(np.clip(dyn["v_hi"], 210.0, 255.0))

        dyn["s_low"] = float(np.clip(dyn["s_low"], 10.0, 80.0))
        dyn["s_mid"] = float(np.clip(dyn["s_mid"], dyn["s_low"], 120.0))
        dyn["s_hi"] = float(np.clip(dyn["s_hi"], dyn["s_mid"], 180.0))

        dyn["rough_hi"] = float(np.clip(dyn["rough_hi"], 5.0, 40.0))
        dyn["ent_hi"] = float(np.clip(dyn["ent_hi"], 3.0, 25.0))
    ...
    def _compute_dynamic_score_threshold_otsu(self, rust_scores_valid: np.ndarray) -> float:
        ...
        Falls back to self.rust_threshold_fallback if not enough data.
        """
        ...
        # If almost constant, Otsu is not meaningful
        ...
        # Keep in a reasonable band so it doesn't go wild on edge cases
    ...
    def _rust_score_for_feature(self, fv: np.ndarray, dyn: Optional[Dict[str, float]] = None) -> float:
        """
        fv: per-segment feature vector
          idx 1=a*, 2=b*, 6=gray_std(roughness proxy), 7=entropy proxy,
          12=H, 13=S, 14=V (OpenCV H 0..180, S/V 0..255)

        dyn: dynamic gates; if None, uses classic constants.
        """
        mean_a = float(fv[1])
        mean_b = float(fv[2])
        ...
        # ---- Stage 0: black rust allowance (dynamic) ----
        is_very_dark = (mean_v < dyn["v_very_dark"])
        warm_enough = (mean_a > (dyn["a_warm"] + 6.0) and mean_b > (dyn["b_warm"] + 6.0))
        textured_enough = (mean_rough > dyn["rough_hi"] or mean_ent > dyn["ent_hi"])

        if is_very_dark and (warm_enough or textured_enough):
            return 0.78

        # ---- Stage 1: vetoes (dynamic) ----
        if mean_v > dyn["v_hi"]:
            return 0.0

        # Must be at least somewhat warm overall (prevents pure gray metal/shadows)
        if mean_a < dyn["a_warm"] or mean_b < dyn["b_warm"]:
            return 0.0

        # Brown rust can be low S; only veto very low S if not red enough and not dark.
        if (mean_s < dyn["s_low"]) and (mean_a < dyn["a_red"]) and (mean_v > (dyn["v_dark"] * 0.55)):
            return 0.0

        # Yellow/brass veto (keep mostly as-is; it's about hue/sat/value interplay)
        if (35.0 < mean_h < 95.0) and (mean_s > max(75.0, dyn["s_hi"])) and (mean_v > max(175.0, dyn["v_dark"])) and (
            mean_a < dyn["a_red"]
        ):
            return 0.0

        # ---- Stage 2: scoring (still your original shape, but with dynamic cut points) ----
        hsv_score = 0.0

        is_rust_hue = (mean_h < 55.0 or mean_h > 165.0)
        if is_rust_hue and mean_s > max(25.0, dyn["s_low"] * 0.8):
            hsv_score += 1.0

        # Saturation: partial credit earlier for browns; dynamic high
        if mean_s > dyn["s_hi"]:
            hsv_score += 1.0
        ...
        # Value: darker tends to be rustier; dynamic v_dark
        ...
        # Texture: same idea, but dynamic scaling anchor
        tex_rough = min(1.0, mean_rough / max(30.0, dyn["rough_hi"] * 2.2))
        tex_ent = min(1.0, mean_ent / max(18.0, dyn["ent_hi"] * 2.0))
        ...
        # Brown-rust allowance: low S but dark + warm/red Lab (dynamic)
        if (mean_s < dyn["s_mid"]) and (mean_v < dyn["v_dark"]) and (mean_a > dyn["a_red"]) and (mean_b > dyn["b_orange"]):
            rust_score = max(rust_score, 0.72)

        # Very dark + warm boost (dynamic)
        if mean_v < (dyn["v_very_dark"] + 10.0) and mean_a > dyn["a_warm"] and mean_b > dyn["b_warm"]:
            rust_score = max(rust_score, 0.75)
    ...
    def _perform_segmentation_analysis(self, crop: np.ndarray, metal_mask_crop: np.ndarray) -> Dict:
        ...
        # Gate to "metal" segments only
        valid = metal_scores > 0.5
        valid_indices = np.where(valid)[0]
        n_valid = int(len(valid_indices))

        # Dynamic gates computed from valid segments (Option A)
        ...
        # Compute rust scores for valid segments
        ...
        # Dynamic final threshold from Otsu on rust_scores (Option B)
        ...
        if n_valid > 0:
            ...
            # Optional: if nothing passes threshold, keep the best valid segment as rust
        ...
        # Build final mask from per-segment predictions
        seg_areas = np.bincount(segments.ravel())
        rust_pixels = 0
        metal_pixels = 0

        final_mask = np.zeros(crop.shape[:2], dtype=np.uint8)
        score_map = np.zeros(crop.shape[:2], dtype=np.float32)  # per-pixel rust score via segment id
        pred_map = np.zeros(crop.shape[:2], dtype=np.uint8)  # per-pixel 0/1 via segment id
        ...
        return {
            ...
            "rust_pred": rust_pred,  # per-segment 0/1
            "score_map": score_map,  # per-pixel score
            "pred_map": pred_map,  # per-pixel pred
    ...
    def visualize_detection(self, results: Dict, save_path: str | None = None) -> plt.Figure:
        ...
        metal_mask = results.get("metal_mask", np.ones(crop.shape[:2], dtype=np.uint8))
        ...
        masked_crop[metal_mask == 0] = 0
        vis_segments = mark_boundaries(cv2.cvtColor(masked_crop, cv2.COLOR_BGR2RGB), segments)

        # Rust-score heatmap inside metal region
        score_vis = cv2.normalize(score_map, None, 0, 1, cv2.NORM_MINMAX)
        score_vis_rgb = plt.cm.inferno(score_vis)[:, :, :3]  # float RGB
        score_vis_rgb[metal_mask == 0] = 0
        ...
        axes[0, 0].imshow(cv2.cvtColor(masked_crop, cv2.COLOR_BGR2RGB))
        axes[0, 0].set_title("1. Metal Input (Masked)")
        axes[0, 0].axis("off")
        ...
        axes[1, 0].imshow(metal_mask, cmap="gray")
        axes[1, 0].set_title("4. Metal Mask")
        axes[1, 0].axis("off")

        axes[1, 1].imshow(score_vis_rgb)
        axes[1, 1].set_title("5. Per-Segment Rust Score (Heatmap)")
        axes[1, 1].axis("off")
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--n_segments", type=int, default=250)
