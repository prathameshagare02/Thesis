...
class FastRustDetector:
    """
    CORRECTED pipeline (SAM3 twice + per-pixel KMeans, NO SLIC) with PASS-2 ROI support gating:
      ...
      PASS 2 (cropped ROI only):
        ...
        6) SAM3 prompt "metal" (ROI)       -> support prior (only for pass-2 background suppression)
        7) Build ROI support mask (suppress blue/red background, keep metal region)
        8) Gate clean/rust evidence by ROI support
        9) Build per-pixel feature vectors from ROI pixels + PASS-2 evidence
       10) KMeans on SUPPORTED ROI pixels
       11) Select rust cluster using prompt-guided cluster scoring
       12) Map ROI rust mask back to original image coordinates

    IMPORTANT:
      - Pass #1 is ONLY for cropping (best metal box).
      - Pass #1 mask is NOT used for rust classification/features/gating.
      - Pass #2 receives the whole cropped ROI box image.
      - PASS-2 metal prompt is used only to suppress background inside the ROI crop.
    """
    ...
    def _print_backend_info(self):
        print("FastRustDetector initialized (PASS1 crop only, PASS2 features/classification + ROI support gating)")
    ...
    def _largest_connected_component(mask_u8: np.ndarray) -> np.ndarray:
        """
        Keeps the largest foreground connected component from a binary uint8 mask.
        """
        if mask_u8 is None:
            return mask_u8
    ...
    def _sam3_best_box_for_prompt(
        ...
        Note:
          We may internally read masks only to recover bbox if xyxy is missing,
          but pass #1 output is used ONLY for cropping.
        """
        if self.sam3_predictor is None:
            return None, 0.0
        ...
        try:
            ...
            # fallback bbox from selected mask
            if best_box is None:
    ...
    def get_best_metal_box(
        ...
        If SAM3 fails -> full image box, score=0.0
        """
        h, w = original.shape[:2]
        box, score = self._sam3_best_box_for_prompt(image_path, self.prompt_metal, expected_hw=(h, w))
        if box is None:
            return (0, 0, w, h), 0.0
    ...
    # ---- PASS #2: ROI support mask (fix background leakage) ----
    def _build_roi_support_mask(
        self,
        crop: np.ndarray,
        metal_evidence: np.ndarray,
        clean_evidence: np.ndarray,
        rust_evidence: np.ndarray,
    ) -> np.ndarray:
        """
        Build a binary support mask for "likely metal region" INSIDE the ROI crop.

        Purpose:
          - suppress obvious background (e.g. blue mats / red fixtures) that SAM3 clean/rust prompts may flag
          - keep rust/clean classification constrained to the metal object in the cropped ROI

        Inputs are PASS-2 evidence maps (ROI only). Pass-1 mask is NOT used.
        """
        h, w = crop.shape[:2]
        if h == 0 or w == 0:
        ...
        # Strong background colors in the example images:
        # - blue tabletop / mat
        # - red plastic/fixture around the metal part
        blue_bg = ((H >= 85) & (H <= 145) & (S >= 40) & (V >= 30))
        red_fixture = ((((H <= 7) | (H >= 173)) & (S >= 80) & (V >= 40)))

        # PASS-2 evidence support (ROI only)
        metal_ev = np.clip(metal_evidence.astype(np.float32), 0.0, 1.0)
        clean_ev = np.clip(clean_evidence.astype(np.float32), 0.0, 1.0)
        rust_ev = np.clip(rust_evidence.astype(np.float32), 0.0, 1.0)
        ev_union = np.maximum.reduce([metal_ev, clean_ev, rust_ev]).astype(np.float32)

        # Start from evidence-driven support, then suppress obvious background colors
        support = (
            (metal_ev >= 0.08)
            | (clean_ev >= 0.10)
            | (rust_ev >= 0.10)
        ...
        # Background suppression (primary fix for blue/red false positives)
        support = support & (~blue_bg) & (~red_fixture)
        ...
        support_u8 = cv2.morphologyEx(support_u8, cv2.MORPH_OPEN, k3)
        support_u8 = cv2.morphologyEx(support_u8, cv2.MORPH_CLOSE, k5)

        # If fragmented, keep largest component (usually the metal object inside ROI)
        if int(support_u8.sum()) > 0:
            support_u8 = self._largest_connected_component(support_u8)
            support_u8 = cv2.dilate(support_u8, k3, iterations=1)

        # Fallback: if support got too small, relax thresholds (still reject blue/red)
        min_pixels = max(20, int(0.01 * h * w))
        if int(support_u8.sum()) < min_pixels:
            relaxed = ((ev_union >= 0.06) | (metal_ev >= 0.04)) & (~blue_bg) & (~red_fixture)
            support_u8 = relaxed.astype(np.uint8)
            support_u8 = cv2.morphologyEx(support_u8, cv2.MORPH_CLOSE, k5)
            if int(support_u8.sum()) > 0:
                support_u8 = self._largest_connected_component(support_u8)
                support_u8 = cv2.dilate(support_u8, k3, iterations=1)

        # Safety fallback: if still empty, use evidence-only (rare)
        if int(support_u8.sum()) == 0:
            support_u8 = (ev_union >= 0.05).astype(np.uint8)

        return (support_u8 > 0).astype(np.uint8)
    ...
    def _compute_pixel_feature_tensor(
        ...
        # PASS #2 SAM3 evidence channels only (already support-gated upstream)
        clean_ev = np.clip(clean_evidence.astype(np.float32), 0.0, 1.0)
        rust_ev = np.clip(rust_evidence.astype(np.float32), 0.0, 1.0)
        ...
        feature_names = [
            ...
            "sam3_rust_evidence",
            "sam3_union_evidence",
            "sam3_rust_minus_clean",
            "sam3_rust_ratio",
        ]
        return features, feature_names

    # ---- KMeans on ROI support pixels only ----
    def _kmeans_pixel_classification(
        ...
        """
        KMeans on SUPPORTED pixels of the ROI box (valid_mask == 1).
        Background pixels are left as cluster=-1 and rust=0.
        """
        ...
        X_all_raw = feature_tensor.reshape(-1, fdim).astype(np.float32)
        X_raw = X_all_raw[valid_flat]

        # Standardize for KMeans (valid pixels only)
        mu = X_raw.mean(axis=0, keepdims=True)
        sigma = X_raw.std(axis=0, keepdims=True)
        sigma[sigma < 1e-6] = 1.0
        X = ((X_raw - mu) / sigma).astype(np.float32)

        # Subsample for fitting, assign all valid later
        n = X.shape[0]
        ...
        if X_train.shape[0] < 2:
            ...
            rust_i = idx["sam3_rust_evidence"]
            clean_i = idx["sam3_clean_evidence"]
            rule = (X_raw[:, rust_i] > X_raw[:, clean_i]).astype(np.uint8)

            cluster_map_flat = np.full((n_pixels,), -1, dtype=np.int16)
            rust_mask_flat = np.zeros((n_pixels,), dtype=np.uint8)
            cluster_map_flat[valid_flat] = rule.astype(np.int16)
            rust_mask_flat[valid_flat] = rule.astype(np.uint8)

            cluster_map = cluster_map_flat.reshape(h, w).astype(np.int16)
            rust_mask = rust_mask_flat.reshape(h, w).astype(np.uint8)
            rust_mask &= valid_mask
        ...
        labels_valid = np.argmin(d2, axis=1).astype(np.int16)
        ...
        for c in range(K):
            ...
            score = (
                2.8 * float(meanv[idx["sam3_rust_evidence"]])
                - 2.2 * float(meanv[idx["sam3_clean_evidence"]])
        ...
        rust_pred = (labels_valid == rust_cluster_id).astype(np.uint8)

        # Light refinement (pass2 evidence + color only) on VALID pixels only
        rust_i = idx["sam3_rust_evidence"]
        clean_i = idx["sam3_clean_evidence"]
        delta_i = idx["sam3_rust_minus_clean"]
        ...
        # Map valid-only labels/predictions back to full ROI
        cluster_map_flat = np.full((n_pixels,), -1, dtype=np.int16)
        rust_mask_flat = np.zeros((n_pixels,), dtype=np.uint8)

        cluster_map_flat[valid_flat] = labels_valid
        rust_mask_flat[valid_flat] = rust_pred

        cluster_map = cluster_map_flat.reshape(h, w).astype(np.int16)
        rust_mask = rust_mask_flat.reshape(h, w).astype(np.uint8)

        # Morphology on final rust mask (then re-apply valid mask)
        kernel = np.ones((3, 3), np.uint8)
    ...
    def _perform_pixel_kmeans_analysis(self, crop: np.ndarray) -> Dict:
        ...
        PASS #2 on the WHOLE ROI crop:
          ...
          - ROI metal support mask to suppress background leakage
          - per-pixel feature vectors
          - KMeans on supported ROI pixels only
        """
        h, w = crop.shape[:2]

        # PASS-2 evidence maps
        ...
        # Additional PASS-2 metal evidence used ONLY to suppress background inside ROI
        self._log(f"SAM3 pass #2 (whole ROI) prompt={self.prompt_metal!r} (ROI support gating)")
        metal_out = self._sam3_prompt_evidence_map(crop, self.prompt_metal, expected_hw=(h, w))

        clean_ev_raw = clean_out["evidence_map"]
        rust_ev_raw = rust_out["evidence_map"]
        metal_ev = metal_out["evidence_map"]

        # Build support mask and gate evidence maps to suppress background
        valid_mask = self._build_roi_support_mask(crop, metal_ev, clean_ev_raw, rust_ev_raw)
        clean_ev = (clean_ev_raw * valid_mask.astype(np.float32)).astype(np.float32)
        rust_ev = (rust_ev_raw * valid_mask.astype(np.float32)).astype(np.float32)

        feature_tensor, feature_names = self._compute_pixel_feature_tensor(crop, clean_ev, rust_ev)

        # KMeans on supported ROI pixels only
        km = self._kmeans_pixel_classification(feature_tensor, feature_names, valid_mask=valid_mask)
        ...
        # diagnostic score map (support-gated)
        score_map = np.clip(0.5 + 0.5 * (rust_ev - clean_ev), 0.0, 1.0).astype(np.float32)
        score_map *= valid_mask.astype(np.float32)

        self._log(
            f"ROI pixels={total_pixels_box}, support={valid_pixels}, rust={rust_pixels} "
            f"({rust_pct_box:.2f}% of ROI box, {rust_pct_valid:.2f}% of support)"
        )

        return dict(
            sam_metal_evidence=metal_ev,
            sam_clean_evidence=clean_ev,
            sam_rust_evidence=rust_ev,
            sam_clean_evidence_raw=clean_ev_raw,
            sam_rust_evidence_raw=rust_ev_raw,
            sam_metal_best_mask=metal_out["best_mask"],
            sam_clean_best_mask=clean_out["best_mask"],
            sam_rust_best_mask=rust_out["best_mask"],
            sam_metal_best_score=float(metal_out["best_score"]),
            sam_clean_best_score=float(clean_out["best_score"]),
            sam_rust_best_score=float(rust_out["best_score"]),
            sam_metal_num_dets=int(metal_out["num_detections"]),
            sam_clean_num_dets=int(clean_out["num_detections"]),
            sam_rust_num_dets=int(rust_out["num_detections"]),
            roi_support_mask=valid_mask.astype(np.uint8),
            pixel_feature_tensor=feature_tensor,      # HxWxF
            pixel_feature_names=feature_names,        # len F
            valid_mask=valid_mask.astype(np.uint8),
            cluster_map=km["cluster_map"],            # HxW int16 (-1 outside support)
    ...
    def analyze(self, image_path: str, interactive: bool = True) -> Dict:
        ...
        if interactive and self.sam3_predictor is not None:
            ...
            metal_box_full, metal_box_score = self.get_best_metal_box(image_path, original)
        else:
            h, w = original.shape[:2]
        ...
        # 3) PASS #2 + per-pixel KMeans on ROI support pixels
        t0 = time.time()
        self._log("Running PASS #2 (clean/rust prompts + ROI support gating) + per-pixel KMeans...")
        ...
        self._log(
            ...
            f"ROI rust coverage (support): {res['rust_percentage_valid']:.2f}% | "
        ...
        return dict(
            ...
            rust_percentage=float(res["rust_percentage_box"]),  # retained original API/meaning
            rust_percentage_valid=float(res["rust_percentage_valid"]),
            timings=timings,
            # pass-2 outputs
            sam_metal_evidence=res["sam_metal_evidence"],
            sam_clean_evidence=res["sam_clean_evidence"],
            sam_rust_evidence=res["sam_rust_evidence"],
            sam_clean_evidence_raw=res["sam_clean_evidence_raw"],
            sam_rust_evidence_raw=res["sam_rust_evidence_raw"],
            roi_support_mask=res["roi_support_mask"],
            sam_metal_best_mask=res["sam_metal_best_mask"],
            sam_clean_best_mask=res["sam_clean_best_mask"],
            sam_rust_best_mask=res["sam_rust_best_mask"],
            sam_metal_best_score=res["sam_metal_best_score"],
            sam_clean_best_score=res["sam_clean_best_score"],
            sam_rust_best_score=res["sam_rust_best_score"],
            sam_metal_num_dets=res["sam_metal_num_dets"],
            ...
            valid_mask=res["valid_mask"],          # now support mask, NOT all ones
            cluster_map=res["cluster_map"],        # -1 outside support
    ...
    def visualize_detection(self, results: Dict, save_path: str | None = None) -> plt.Figure:
        ...
        valid_mask = results.get("valid_mask", np.ones(crop.shape[:2], dtype=np.uint8))
        ...
        # Stage 4: pass2 prompt evidence (clean=green, rust=red), support-gated
        stage4 = crop.copy()
        ...
        # Optionally dim unsupported pixels to make the fix visually obvious
        unsupported = (valid_mask == 0)
        stage4_dim = stage4.copy()
        stage4_dim[unsupported] = (stage4_dim[unsupported] * 0.55).astype(np.uint8)
        stage4 = stage4_dim

        # Stage 5: KMeans clusters + final rust mask in ROI (only supported pixels)
        stage5 = crop.copy()
        ...
        fig.suptitle(
            "Rust Detection Stages (SAM3 pass1 crop-only -> SAM3 pass2 clean/rust on ROI + ROI support gating -> per-pixel KMeans)\n"
        ...
        titles = [
            ...
            "4) SAM3 pass #2 evidence on ROI (clean=green, rust=red) + background suppression",
            "5) KMeans pixel clusters on ROI support + final rust mask",
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Rust Detection: SAM3 pass1 metal ROI crop-only -> SAM3 pass2 clean/rust evidence on ROI + background suppression -> per-pixel KMeans"
    )
    ...
    # Prompts
    p.add_argument("--prompt_metal", type=str, default="metal", help="SAM3 prompt for pass #1 ROI extraction and pass-2 ROI support gating.")
    ...
    p.add_argument(
        ...
        type=int,
        default=80000,
        help="Max sampled pixels used to fit KMeans; all valid ROI pixels are assigned afterward.",
    )
...
def main():
    ...
    if bool(args.save_features):
        ...
        np.savez_compressed(
            ...
            sam3_metal_evidence=results["sam_metal_evidence"].astype(np.float32),
            sam3_clean_evidence=results["sam_clean_evidence"].astype(np.float32),
            sam3_rust_evidence=results["sam_rust_evidence"].astype(np.float32),
            sam3_clean_evidence_raw=results["sam_clean_evidence_raw"].astype(np.float32),
            sam3_rust_evidence_raw=results["sam_rust_evidence_raw"].astype(np.float32),
            roi_support_mask=results["roi_support_mask"].astype(np.uint8),
            sam3_clean_best_mask=results["sam_clean_best_mask"].astype(np.uint8),
            sam3_rust_best_mask=results["sam_rust_best_mask"].astype(np.uint8),

            # Per-pixel features (whole ROI tensor, but valid_mask defines supported pixels)
            pixel_feature_tensor=results["pixel_feature_tensor"].astype(np.float32),  # HxWxF
            pixel_feature_names=feature_names_arr,  # [F]

            # KMeans / output maps
            valid_mask=results["valid_mask"].astype(np.uint8),  # support mask (not necessarily all ones)
            cluster_map=results["cluster_map"].astype(np.int16),  # -1 outside support
            ...
            rust_percentage_support=np.array([results["rust_percentage_valid"]], dtype=np.float32),
        )
        ...
        print("CONFIRMED: pass #1 only crops ROI; pass #2 runs on the whole cropped ROI box with ROI support gating to suppress background.")
