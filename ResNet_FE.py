...
class MetalSegmenter:
    def __init__(self, show: bool = True):
        ...
        self.show = show               # whether to display visualizations
    ...
    def load_sam2_model(self):
        ...
        try:
            ...
            # Move SAM model to GPU (MPS for Apple Silicon)
            self.model.to(self.device)
            print(f"SAM 2 model loaded successfully on {self.device.upper()}!")
            return True
    ...
    # ------------------------------------------------------------------
    def visualize_tsne_clusters(self, embeddings, centers, metal_mask=None, n_clusters=2, show=None):
        """
        ...
        if self.image_path:
            ...
            save_path = f"results/ResNet/{base_name}_tsne.png"
    ...
    def analyze_embedding_structure(self, show=None):
        """
        ...
        if embeddings is not None and centers is not None:
            ...
            self.visualize_tsne_clusters(embeddings, centers, metal_mask=metal_mask, show=show)
        else:
    ...
    # ------------------------------------------------------------------
    def rust_resnet_embedding_based(self, patch_size=64, stride=32, max_patches=1000):
        """
        ...
        # Estimate number of patches and adjust stride if too many
        n_rows = (ymax - ymin) // stride + 1
        n_cols = (xmax - xmin) // stride + 1
        est_patches = n_rows * n_cols
        if est_patches > max_patches:
            # Increase stride to limit patches
            scale = int(np.ceil(np.sqrt(est_patches / max_patches)))
            stride = stride * scale
            print(f"Adjusted stride to {stride} to limit patches (estimated {est_patches} -> ~{max_patches})")

        print(f"Running ResNet-based rust detection (patch_size={patch_size}, stride={stride})...")

        patch_feats = []
        patch_coords = []   # (y, y2, x, x2)
            ...
            for y in range(ymin, ymax + 1, stride):
                for x in range(xmin, xmax + 1, stride):
                    ...
                    if patch_count % 100 == 0:
                        print(f"  Processed {patch_count} patches...", end="\r")

        print(f"  Total patches processed: {len(patch_feats)}                    ")
    ...
    def run_rust_detection(self, show=None):
        ...
        if self.rust_mask is not None:
            self.show_rust_results(show=show)
        else:
            print("Rust detection failed or skipped.")

    def show_rust_results(self, show=None):
        ...
        if self.image_path:
            os.makedirs("results/ResNet", exist_ok=True)
            base_name = os.path.splitext(os.path.basename(self.image_path))[0]
            save_path = f"results/ResNet/{base_name}_result.png"
            fig.savefig(save_path, dpi=300, bbox_inches='tight')
            print(f"Rust detection visualization saved to: {save_path}")
        plt.close(fig)
    ...
    def sam2_predict(self, points, labels):
        ...
        try:
            ...
            results = self.model.predict(image_rgb, points=points, labels=labels, device=self.device)
            if (
                results
                and len(results) > 0
    ...
    # FINAL STEP WRAPPERS
    # ------------------------------------------------------------------
    def show_results(self, show=None):
        ...
        self.run_rust_detection(show=show)

    def run(self, image_path=None, show=None):
        """
        Main workflow.
        
        Args:
            image_path: Path to image file. If None, opens file dialog.
            show: Whether to display visualizations. If None, uses self.show.
        """
        if show is None:
            show = self.show
        ...
        self.analyze_embedding_structure(show=show)
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Metal Segmentation + ResNet Embeddings + t-SNE Analysis")
    p.add_argument("--image", type=str, default="", help="Path to image; if empty, file dialog opens.")
    p.add_argument("--show", type=int, default=1, help="1=display visualization windows; 0=no display.")
    return p
...
if __name__ == "__main__":
    ...
    segmenter = MetalSegmenter(show=bool(args.show))
    segmenter.run(image_path=image_path)
