...
class CorrosionSegmenter:
    ...
    def save_results(self):
        """Save corrosion analysis results"""
        if self.image is None or self.mask is None:
        ...
        root = tk.Tk()
        root.withdraw()
        save_dir = filedialog.askdirectory(title="Select directory to save corrosion analysis")
        if not save_dir:
            return
        ...
        # Save images
        cv2.imwrite(os.path.join(save_dir, f"{base_name}_original.jpg"), self.original_image)
        cv2.imwrite(os.path.join(save_dir, f"{base_name}_corrosion_mask.png"), self.mask)
        
        corrosion_only = self.original_image.copy()
        corrosion_only[self.mask == 0] = 0
        cv2.imwrite(os.path.join(save_dir, f"{base_name}_corrosion_only.png"), corrosion_only)
        
        white_bg = self.original_image.copy()
        white_bg[self.mask == 0] = [255, 255, 255]
        cv2.imwrite(os.path.join(save_dir, f"{base_name}_white_bg.jpg"), white_bg)
        ...
        messagebox.showinfo("Success", f"Corrosion analysis saved to:\n{save_dir}")
        root.destroy()
    ...
    def run(self):
        ...
        save = input("Save corrosion analysis results? (y/n): ").lower()
        if save == 'y':
            self.save_results()
        
        print("Corrosion analysis completed!")
