...
class MetalSegmenter:
    ...
    def extract_patch_embeddings(
        ...
        print("Extracting patch embeddings (purely image-based, METAL ONLY)...")
        with torch.no_grad():
    ...
    def visualize_tsne_clusters(self, embeddings, centers, metal_mask=None, n_clusters=2):
        ...
        Uses ONLY image embeddings for t-SNE / KMeans, but
        uses CLIP text prompts to decide which cluster is rust vs metal.
        """
        ...
        # -------- DATA-DRIVEN rust vs metal MAPPING --------
        rust_cluster = None
        metal_cluster = None
        if self.clip_model is not None:
            ...
            img_feats_t = torch.from_numpy(embeddings).to(self.device)  # (N, D)
            img_feats_t = F.normalize(img_feats_t, dim=-1)

            sims = img_feats_t @ text_features.T  # (N, 2)
            rust_sims = sims[:, 0].cpu().numpy()
            clean_sims = sims[:, 1].cpu().numpy()
            diff = rust_sims - clean_sims          # positive => more rust-like
            ...
            for k in range(n_clusters):
                ...
                if not np.any(mask_k):
                    ...
                    continue
                score_k = diff[mask_k].mean()
                cluster_scores.append(score_k)
                print(f"Cluster {k}: mean(rust_sim - clean_sim) = {score_k:.4f}")

            rust_cluster = int(np.argmax(cluster_scores))
            metal_cluster = 1 - rust_cluster if n_clusters == 2 else None

            print(f"Data-driven mapping: rust_cluster = {rust_cluster}, metal_cluster = {metal_cluster}")
        else:
            print("CLIP model not available inside visualize_tsne_clusters; using generic cluster names.")
        ...
        fig.suptitle("Patch Embedding Structure on METAL REGION (CLIP + t-SNE)",
        ...
        print("  - CLIP decides which cluster is 'rust' vs 'metal' based on text similarity.")
        print("  - The 2D plot shows the structure of patch embeddings restricted to the metal region.")
    ...
    # ------------------------------------------------------------------
    # EMBEDDING-BASED ZERO-SHOT RUST DETECTOR (CLIP)
    # ------------------------------------------------------------------
    def rust_clip_embedding_based(self, patch_size=8, stride=4):
        """
        Embedding-based zero-shot rust detector using CLIP.
        Uses small 8x8 patches (default) with stride 4.
        """
        if self.clip_model is None or self.clip_preprocess is None:
    ...
    def run_rust_detection(self):
        ...
        # Explicitly use 8x8 patches with stride 4
    ...
    def run(self):
        ...
        Main workflow.
        """
        print("METAL SEGMENTATION + CLIP EMBEDDINGS + METAL-ONLY t-SNE STRUCTURE ANALYSIS")
        print("=" * 80)
