...
class FastRustDetector:
    """
    Flow:
      1) Run SAM3 text prompt(s) for metal and choose the MAX-confidence detection overall.
         - Keep that box + score (for further processing / reporting).
         - Use that box as ROI crop for next stage.
      2) Inside ROI crop, compute SLIC superpixels.
    ...
    def __init__(
        ...
        device: str = "",
        bpe_path: str = "",
        metal_prompts: Optional[List[str]] = None,
        ...
        # IMPORTANT: these prompts are ONLY for ROI (metal box), not rust
        self.metal_prompts = metal_prompts or ["metal", "metal plate", "metal surface"]
    ...
    def _print_backend_info(self):
        ...
        print("  SAM3 metal prompts (ROI selection):")
        for p in self.metal_prompts:
            print(f"    - {p!r}")
    ...
    def get_best_metal_roi(
        self, image_path: str
    ) -> Tuple[np.ndarray, Tuple[int, int, int, int], float, str]:
        """
        Runs multiple metal prompts and selects the overall best detection by score.
        Returns:
          metal_mask_full (HxW uint8),
          metal_box_full (x1,y1,x2,y2),
          metal_score (float),
          prompt_used (str)

        If SAM3 fails for all prompts => full image ROI with score 0.0
        """
        ...
        for prompt in self.metal_prompts:
            m, b, s = self._sam3_best_detection_for_prompt(image_path, prompt)
            if m is None or b is None or not np.any(m):
                continue
            if s > best_score:
                best_mask, best_box, best_score, best_prompt = m, b, float(s), prompt
        ...
        if best_mask is None or best_box is None:
            ...
            H, W = img.shape[:2]
            best_mask = np.ones((H, W), dtype=np.uint8)
            best_box = (0, 0, W, H)
            best_score = 0.0
            best_prompt = "FULL_IMAGE_FALLBACK"

        return best_mask.astype(np.uint8), best_box, float(best_score), best_prompt
    ...
    def analyze(self, image_path: str) -> Dict:
        ...
        # 1) SAM3: best metal detection across multiple prompts (max confidence overall)
        t0 = time.time()
        self._log("SAM3: selecting best ROI across metal prompts (max score box overall)...")
        metal_mask_full, metal_box_full, metal_box_score, metal_prompt_used = self.get_best_metal_roi(image_path)
        timings["sam3_metal"] = time.time() - t0

        x1, y1, x2, y2 = metal_box_full

        # 2) Crop using the best box + padding
        t0 = time.time()
        pad = self.roi_pad
        x1p = max(0, x1 - pad)
        y1p = max(0, y1 - pad)
        ...
        # 3) SLIC + Otsu rust scoring inside ROI
        t0 = time.time()
        ...
        self._log(
            f"Metal prompt used: {metal_prompt_used!r} | Metal box score: {metal_box_score:.2f} | "
        ...
        return dict(
            ...
            metal_prompt_used=metal_prompt_used,     # very useful for debugging
        )
    ...
    def visualize_detection(self, results: Dict, save_path: str | None = None) -> plt.Figure:
        ...
        p_used = results.get("metal_prompt_used", "")

        # left: ROI crop, show metal mask in blue overlay (sanity check)
        roi_vis = crop.copy()
        ...
        fig.suptitle(
            f"Rust Detection | Metal prompt={p_used!r} | Best metal score={bscore:.2f} | Segments={self.n_segments}\n"
...
# ----------------------------- CLI + Main -----------------------------
def _parse_prompts(arg_list: List[str], comma_str: str) -> List[str]:
    """
    Accept prompts either as:
      --metal_prompt "metal" --metal_prompt "metal plate"
    OR as:
      --metal_prompts "metal, metal plate, metal surface"
    """
    prompts: List[str] = []
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Rust Detection: SAM3 best metal box (max score across prompts) -> SLIC superpixels -> Otsu threshold"
    )
    ...
    # Prompts for ROI selection (metal box)
    p.add_argument(
        "--metal_prompt",
        action="append",
        default=[],
        help="Repeatable. Text prompt used for selecting metal ROI. Example: --metal_prompt 'metal plate'",
    )
    p.add_argument(
        "--metal_prompts",
        type=str,
        default="",
        help="Comma-separated metal prompts alternative. Example: --metal_prompts 'metal, metal plate, metal surface'",
    )
...
def main():
    ...
    metal_prompts = _parse_prompts(args.metal_prompt, args.metal_prompts)
    if not metal_prompts:
        metal_prompts = ["metal", "metal plate", "metal surface"]
