...
class MetalSegmenter:
    ...
    def extract_patch_embeddings(
        ...
        # ---------- PRINT EMBEDDING VECTORS HERE ----------
        print("\n=== RAW CLIP PATCH EMBEDDINGS ===")
        print(f"Shape: N={embeddings.shape[0]}, D={embeddings.shape[1]}")
        for i, vec in enumerate(embeddings):
            print(f"\nPatch {i:03d} embedding (len={len(vec)}):")
    ...
    # ------------------------------------------------------------------
    # T-SNE VISUALIZATION USING ONLY PATCH EMBEDDINGS
    # ------------------------------------------------------------------
    def visualize_tsne_clusters(self, embeddings, centers, n_clusters=2):
        ...
        Colors are kept consistent between the t-SNE plot and the image.
        """
        ...
        # Take the first n_clusters colors from tab10
        ...
        # Manual legend with same colors as on the image
        for k in range(n_clusters):
            ax0.scatter([], [], color=cluster_colors[k], label=str(k))
        ax0.legend(title="Clusters", loc="best")
    ...
    # ------------------------------------------------------------------
    def rust_clip_embedding_based(self, patch_size=16, stride=8):
        """
    ...
    def run(self):
        ...
        For supervisor's request, focus on the t-SNE / clustering plots
        produced by analyze_embedding_structure().
        """
