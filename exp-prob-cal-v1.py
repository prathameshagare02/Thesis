...
class RustProbabilityCalibrator:
    """
    Calibrates raw combined rust scores to true probabilities.
    Uses the KMeans binary mask as pseudo-ground-truth.
    
    The raw score combines:
    - SAM3 evidence (rust/clean)
    - Color features (redness, brownness)
    - Texture features (gradient, local std)
    ...
    def __init__(self, method: str = 'isotonic'):
        ...
        self.n_samples_fitted = 0
        self.raw_score_range = (0.0, 1.0)
    
    def fit(self, raw_scores: np.ndarray, labels: np.ndarray, sample_weight: Optional[np.ndarray] = None):
        """
        Fit calibrator on raw scores and binary labels.
        
        Args:
            raw_scores: 1D array of raw combined scores in [0,1]
            labels: 1D array of binary labels (0=clean, 1=rust)
        ...
        valid = np.isfinite(raw_scores) & np.isin(labels, [0, 1])
        raw_scores = raw_scores[valid]
        labels = labels[valid]
        if sample_weight is not None:
            sample_weight = np.asarray(sample_weight).ravel()[valid]
        
        if len(raw_scores) < 10:
            raise ValueError(f"Need at least 10 samples for calibration, got {len(raw_scores)}")
        
        self.raw_score_range = (float(raw_scores.min()), float(raw_scores.max()))
        
        if self.method == 'isotonic':
            self.calibrator = IsotonicRegression(y_min=0.0, y_max=1.0, out_of_bounds='clip')
            self.calibrator.fit(raw_scores, labels, sample_weight=sample_weight)
        elif self.method == 'sigmoid':
            self.calibrator = LogisticRegression(solver='lbfgs', max_iter=1000)
            self.calibrator.fit(raw_scores.reshape(-1, 1), labels, sample_weight=sample_weight)
        else:
            raise ValueError(f"Unknown calibration method: {self.method}")
        
        self.is_fitted = True
        self.n_samples_fitted = len(raw_scores)
    ...
    def predict_proba(self, raw_scores: np.ndarray) -> np.ndarray:
        ...
        raw_scores = np.asarray(raw_scores).astype(np.float64)
        original_shape = raw_scores.shape
        raw_flat = raw_scores.ravel()
        
        if self.method == 'isotonic':
            calibrated = self.calibrator.predict(raw_flat)
        else:  # sigmoid
            calibrated = self.calibrator.predict_proba(raw_flat.reshape(-1, 1))[:, 1]
    ...
    def save(self, path: str):
        ...
        with open(path, 'wb') as f:
            pickle.dump({
                ...
                'raw_score_range': self.raw_score_range,
            }, f)
    ...
    def load(cls, path: str) -> 'RustProbabilityCalibrator':
        ...
        obj.n_samples_fitted = data['n_samples_fitted']
        obj.raw_score_range = data.get('raw_score_range', (0.0, 1.0))
        return obj
    
    def compute_calibration_error(self, raw_scores: np.ndarray, labels: np.ndarray, n_bins: int = 10) -> Dict:
        """Compute Expected Calibration Error (ECE)."""
        calibrated = self.predict_proba(raw_scores)
        labels = np.asarray(labels).ravel()
        calibrated = calibrated.ravel()
        ...
        for i in range(n_bins):
            ...
            if n_in_bin > 0:
                ...
                ece += (n_in_bin / len(labels)) * abs(avg_conf - avg_acc)
                bin_stats.append({'bin': i, 'n': int(n_in_bin), 'conf': float(avg_conf), 'acc': float(avg_acc)})
...
class FastRustDetector:
    ...
    def _perform_pixel_kmeans_analysis(self, crop: np.ndarray) -> Dict:
        ...
        # diagnostic score map (SAM3-only, for comparison)
        score_map = np.clip(0.5 + 0.5 * (rust_ev - clean_ev), 0.0, 1.0).astype(np.float32)
        
        # COMBINED raw score using all features (same weights as cluster selection)
        # This is the score we calibrate for probability output
        h, w = crop.shape[:2]
        idx = {name: i for i, name in enumerate(feature_names)}
        F = feature_tensor.reshape(-1, len(feature_names))
        
        combined_raw = (
            2.8 * F[:, idx["sam2_rust_evidence"]]
            - 2.2 * F[:, idx["sam2_clean_evidence"]]
            + 0.45 * F[:, idx["redness"]]
            + 0.40 * F[:, idx["brownness"]]
            + 0.18 * F[:, idx["texture_norm"]]
            + 0.10 * F[:, idx["grad_norm"]]
            + 0.10 * F[:, idx["S_norm"]]
            - 0.05 * F[:, idx["V_norm"]]
        )
        # Normalize to [0,1] using sigmoid
        combined_score_map = (1.0 / (1.0 + np.exp(-combined_raw))).reshape(h, w).astype(np.float32)
        ...
        return dict(
            ...
            score_map=score_map,
            combined_score_map=combined_score_map,  # Combined features raw probability
    ...
    def analyze(self, image_path: str, interactive: bool = True) -> Dict:
        ...
        # Map combined score to full image (0 outside ROI)
        full_combined_score = np.zeros((H, W), dtype=np.float32)
        full_combined_score[y1p:y2p, x1p:x2p] = res["combined_score_map"]
        ...
        return dict(
            ...
            full_combined_score=full_combined_score,  # Combined raw probability (full image)
            crop_mask=res["crop_mask"],
            combined_score_map=res["combined_score_map"],  # Combined raw probability (ROI)
    ...
    def visualize_with_calibration(
        self,
        results: Dict,
        calibrator: Optional[RustProbabilityCalibrator] = None,
        save_path: Optional[str] = None,
        show: bool = True,
    ) -> plt.Figure:
        """
        Extended 8-panel visualization including:
        - Raw combined probability (from all features)
        - Calibrated probability (if calibrator provided)
        """
        original = results["original"]
        crop = results["crop"]
        full_mask = results["full_mask"]
        crop_mask = results["crop_mask"]
        combined_score = results.get("combined_score_map", results["score_map"])
        ...
        # Compute calibrated probabilities if calibrator provided
        if calibrator is not None and calibrator.is_fitted:
            calibrated_prob = calibrator.predict_proba(combined_score)
        else:
            calibrated_prob = combined_score  # fallback to raw

        # Panel 1: Input
        panel1 = original.copy()

        # Panel 2: Metal ROI box
        panel2 = original.copy()
        cv2.rectangle(panel2, (bx1, by1), (bx2, by2), (255, 0, 0), 2)

        # Panel 3: Cropped ROI
        panel3 = crop.copy()

        # Panel 4: SAM3 evidence (clean=green, rust=red)
        panel4 = crop.copy()
        ev_vis = np.zeros_like(crop, dtype=np.uint8)
        ev_vis[:, :, 1] = np.clip(clean_ev * 255.0, 0, 255).astype(np.uint8)
        ev_vis[:, :, 2] = np.clip(rust_ev * 255.0, 0, 255).astype(np.uint8)
        panel4 = cv2.addWeighted(panel4, 0.6, ev_vis, 0.5, 0)

        # Panel 5: KMeans clusters
        panel5 = crop.copy()
        km_overlay = panel5.copy()
        if clean_cluster_id >= 0:
            km_overlay[(cluster_map == clean_cluster_id)] = [0, 255, 0]
        if rust_cluster_id >= 0:
            km_overlay[(cluster_map == rust_cluster_id)] = [0, 0, 255]
        panel5 = cv2.addWeighted(panel5, 0.55, km_overlay, 0.45, 0)

        # Panel 6: Final rust overlay
        panel6 = original.copy()
        rust_overlay = panel6.copy()
        rust_overlay[full_mask == 1] = [0, 0, 255]
        panel6 = cv2.addWeighted(panel6, 0.6, rust_overlay, 0.4, 0)
        cv2.rectangle(panel6, (bx1, by1), (bx2, by2), (255, 0, 0), 2)

        # Create figure
        fig, axes = plt.subplots(2, 4, figsize=(22, 10))
        
        title_suffix = " (calibrated)" if (calibrator is not None and calibrator.is_fitted) else " (raw)"
        fig.suptitle(
            f"Rust Detection with Combined Probability{title_suffix}\n"
            f"Metal score={bscore:.2f} | ROI rust coverage={rust_percentage:.1f}%",
            fontsize=12,
        )

        # Row 1: Original pipeline stages
        axes[0, 0].imshow(cv2.cvtColor(panel1, cv2.COLOR_BGR2RGB))
        axes[0, 0].set_title("1) Input")
        axes[0, 0].axis("off")

        axes[0, 1].imshow(cv2.cvtColor(panel2, cv2.COLOR_BGR2RGB))
        axes[0, 1].set_title("2) Metal ROI box")
        axes[0, 1].axis("off")

        axes[0, 2].imshow(cv2.cvtColor(panel3, cv2.COLOR_BGR2RGB))
        axes[0, 2].set_title("3) Cropped ROI")
        axes[0, 2].axis("off")

        axes[0, 3].imshow(cv2.cvtColor(panel4, cv2.COLOR_BGR2RGB))
        axes[0, 3].set_title("4) SAM3 evidence (green=clean, red=rust)")
        axes[0, 3].axis("off")

        # Row 2: KMeans, Mask, Raw Prob, Calibrated Prob
        axes[1, 0].imshow(cv2.cvtColor(panel5, cv2.COLOR_BGR2RGB))
        axes[1, 0].set_title("5) KMeans clusters")
        axes[1, 0].axis("off")

        axes[1, 1].imshow(cv2.cvtColor(panel6, cv2.COLOR_BGR2RGB))
        axes[1, 1].set_title("6) Final rust mask (red)")
        axes[1, 1].axis("off")

        # Raw combined probability heatmap
        im_raw = axes[1, 2].imshow(combined_score, cmap='RdYlGn_r', vmin=0, vmax=1)
        axes[1, 2].set_title(f"7) Raw combined P(rust)\nmean={combined_score.mean():.3f}")
        axes[1, 2].axis("off")
        plt.colorbar(im_raw, ax=axes[1, 2], fraction=0.046)

        # Calibrated probability heatmap
        im_cal = axes[1, 3].imshow(calibrated_prob, cmap='RdYlGn_r', vmin=0, vmax=1)
        cal_label = "8) Calibrated P(rust)" if (calibrator is not None and calibrator.is_fitted) else "8) Raw P(rust)"
        axes[1, 3].set_title(f"{cal_label}\nmean={calibrated_prob.mean():.3f}")
        axes[1, 3].axis("off")
        plt.colorbar(im_cal, ax=axes[1, 3], fraction=0.046)
        ...
        if save_path:
            ...
            if out_dir:
                os.makedirs(out_dir, exist_ok=True)
            fig.savefig(save_path, dpi=300)
            self._log(f"Calibration visualization saved to: {save_path}")
    ...
    def save_calibration_outputs(
        ...
        """
        Save calibration-related outputs to files.
        
        Outputs:
        - {basename}_probability.png: Calibrated probability heatmap
        - {basename}_probability.npz: Raw + calibrated probability arrays
        - {basename}_calibration_stats.json: Statistics about the probabilities
        - {basename}_probability_histogram.png: Histogram of probabilities
        """
        import json
        ...
        combined_score = results.get("combined_score_map", results["score_map"])
        full_combined_score = results.get("full_combined_score")
        crop_mask = results["crop_mask"]
        
        # Compute calibrated probabilities
        if calibrator is not None and calibrator.is_fitted:
            calibrated_prob = calibrator.predict_proba(combined_score)
            if full_combined_score is not None:
                full_calibrated_prob = calibrator.predict_proba(full_combined_score)
            else:
                full_calibrated_prob = None
        else:
            calibrated_prob = combined_score
            full_calibrated_prob = full_combined_score
        
        # 1. Save probability heatmap PNG
        png_path = os.path.join(output_dir, f"{basename}_probability.png")
        fig, axes = plt.subplots(1, 3, figsize=(15, 5))
        
        axes[0].imshow(combined_score, cmap='RdYlGn_r', vmin=0, vmax=1)
        axes[0].set_title(f"Raw P(rust)\nmean={combined_score.mean():.3f}")
        axes[0].axis("off")
        
        axes[1].imshow(calibrated_prob, cmap='RdYlGn_r', vmin=0, vmax=1)
        cal_label = "Calibrated" if (calibrator is not None and calibrator.is_fitted) else "Raw"
        axes[1].set_title(f"{cal_label} P(rust)\nmean={calibrated_prob.mean():.3f}")
        axes[1].axis("off")
        
        # Show mask for comparison
        axes[2].imshow(crop_mask, cmap='gray', vmin=0, vmax=1)
        axes[2].set_title(f"KMeans mask\ncoverage={crop_mask.mean()*100:.1f}%")
        axes[2].axis("off")
        
        plt.tight_layout()
        fig.savefig(png_path, dpi=200)
        plt.close(fig)
        saved_files['probability_png'] = png_path
        
        # 2. Save probability arrays NPZ
        npz_path = os.path.join(output_dir, f"{basename}_probability.npz")
        save_dict = {
            'raw_probability': combined_score.astype(np.float32),
            'calibrated_probability': calibrated_prob.astype(np.float32),
            'kmeans_mask': crop_mask.astype(np.uint8),
        }
        if full_combined_score is not None:
            save_dict['full_raw_probability'] = full_combined_score.astype(np.float32)
        if full_calibrated_prob is not None:
            save_dict['full_calibrated_probability'] = full_calibrated_prob.astype(np.float32)
        
        np.savez_compressed(npz_path, **save_dict)
        saved_files['probability_npz'] = npz_path
        ...
        stats = {
            'raw_probability': {
                ...
                'median': float(np.median(combined_score)),
            },
            'calibrated_probability': {
                'mean': float(calibrated_prob.mean()),
                'std': float(calibrated_prob.std()),
                'min': float(calibrated_prob.min()),
                'max': float(calibrated_prob.max()),
                'median': float(np.median(calibrated_prob)),
            },
            'kmeans_mask_coverage': float(crop_mask.mean()),
            'calibrator_fitted': calibrator is not None and calibrator.is_fitted,
            'calibrator_method': calibrator.method if calibrator else None,
            'calibrator_n_samples': calibrator.n_samples_fitted if calibrator else 0,
        }
        
        json_path = os.path.join(output_dir, f"{basename}_calibration_stats.json")
        with open(json_path, 'w') as f:
            json.dump(stats, f, indent=2)
        saved_files['stats_json'] = json_path
        
        # 4. Save probability histogram
        hist_path = os.path.join(output_dir, f"{basename}_probability_histogram.png")
        fig, axes = plt.subplots(1, 2, figsize=(12, 4))
        
        axes[0].hist(combined_score.ravel(), bins=50, alpha=0.7, color='red', density=True)
        axes[0].axvline(combined_score.mean(), color='black', linestyle='--', label=f'mean={combined_score.mean():.3f}')
        axes[0].set_xlabel("Raw P(rust)")
        axes[0].set_ylabel("Density")
        axes[0].set_title("Raw Combined Probability Distribution")
        axes[0].legend()
        
        axes[1].hist(calibrated_prob.ravel(), bins=50, alpha=0.7, color='blue', density=True)
        axes[1].axvline(calibrated_prob.mean(), color='black', linestyle='--', label=f'mean={calibrated_prob.mean():.3f}')
        axes[1].set_xlabel(f"{cal_label} P(rust)")
        axes[1].set_ylabel("Density")
        axes[1].set_title(f"{cal_label} Probability Distribution")
        axes[1].legend()
        
        plt.tight_layout()
        fig.savefig(hist_path, dpi=150)
        plt.close(fig)
        saved_files['histogram_png'] = hist_path
        
        self._log(f"Saved calibration outputs to {output_dir}")
        return saved_files
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    # Calibration
    p.add_argument("--calibrator_path", type=str, default="", help="Path to trained calibrator.pkl; if empty, use raw scores")
    p.add_argument("--save_calibration_outputs", type=int, default=0, help="Save calibration outputs (PNG, NPZ, JSON, histogram)")
    p.add_argument("--train_calibrator", type=int, default=0, help="Train calibrator from this image's KMeans output")
    p.add_argument("--calibrator_method", type=str, default="isotonic", choices=["isotonic", "sigmoid"], help="Calibration method")
...
def main():
    ...
    # Load or train calibrator
    calibrator = None
    if args.calibrator_path and os.path.exists(args.calibrator_path):
        print(f"Loading calibrator from: {args.calibrator_path}")
        calibrator = RustProbabilityCalibrator.load(args.calibrator_path)
        print(f"  Method: {calibrator.method}, fitted on {calibrator.n_samples_fitted} samples")
    
    if bool(args.train_calibrator):
        # Train calibrator using KMeans mask as pseudo-labels
        print("Training calibrator from KMeans output...")
        combined_score = results.get("combined_score_map", results["score_map"])
        crop_mask = results["crop_mask"]
        
        calibrator = RustProbabilityCalibrator(method=args.calibrator_method)
        calibrator.fit(combined_score.ravel(), crop_mask.ravel())
        
        cal_out_path = os.path.join(out_dir, f"{base}_calibrator.pkl")
        calibrator.save(cal_out_path)
        print(f"  Saved calibrator to: {cal_out_path}")
        print(f"  Fitted on {calibrator.n_samples_fitted} samples")

    # Visualization
    img_out = os.path.join(out_dir, f"{base}_stages.png")
    if calibrator is not None:
        # Use extended visualization with calibration
        detector.visualize_with_calibration(results, calibrator=calibrator, save_path=img_out, show=not args.no_show)
    else:
        detector.visualize_detection(results, save_path=img_out, show=not args.no_show)
    
    # Save calibration outputs
    if bool(args.save_calibration_outputs):
        saved = detector.save_calibration_outputs(results, out_dir, base, calibrator=calibrator)
        print(f"Saved calibration outputs: {list(saved.keys())}")
