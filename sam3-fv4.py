...
class FastRustDetector:
    """
    CORRECTED pipeline (SAM3 twice + per-pixel KMeans with metal mask filtering):

      PASS 1 (full image):
        1) SAM3 prompt "metal"
        2) Select detection with MAX confidence
        3) Extract best metal box AND mask
        4) Crop FULL IMAGE using that best metal box (with padding for context)

      PASS 2 (cropped ROI with padding):
        5) SAM3 prompt "clean shiny metal" -> per-pixel evidence map in ROI
        6) SAM3 prompt "rust corrosion"    -> per-pixel evidence map in ROI
        7) Build per-pixel feature vectors from:
             - ROI image pixels
             - PASS-2 SAM3 evidence (clean/rust)
        8) K-means on METAL PIXELS ONLY (filtered by PASS #1 mask)
        9) Select rust cluster using prompt-guided cluster scoring
        10) Map ROI rust mask back to original image coordinates

    IMPORTANT:
      - Pass #1 provides the metal box for cropping AND the metal mask for filtering.
      - Pass #1 mask is used to exclude background pixels from classification.
      - Pass #2 runs on the cropped ROI (with padding) but only classifies pixels within the metal mask.
      - ROI padding (default 50px) is crucial for giving SAM3 enough context in PASS #2.
    """
    ...
    def __init__(
        ...
        rust_prompt: str = "rust corrosion",
        # KMeans
        kmeans_k: int = 2,
    ...
    def _print_backend_info(self):
        ...
        print(f"  ROI padding: {self.roi_pad}px (gives SAM3 more context in PASS #2)")
    ...
    def _sam3_best_box_for_prompt(
        ...
        try:
            ...
            morph_result = self.apply_morphological_operations(best_mask_u8)
            if morph_result is not None:
                best_mask_u8 = morph_result
            ...
            return best_box, float(best_score), best_mask_u8
    ...
    def get_best_metal_box(
        ...
        Returns:
          ...
          metal_score float
          metal_mask (H, W) uint8 or None

        If SAM3 fails -> full image box, score=0.0, None mask
        """
    ...
    def _compute_pixel_feature_tensor(
        ...
        # Rust-oriented color priors (improved to capture darker rust/corrosion)
        # Redness: capture both bright red rust and darker reddish-brown
        redness = np.clip((a_centered + 15.0) / 70.0, 0.0, 1.0).astype(np.float32)
        # Brownness: capture yellowish-brown and reddish-brown rust
        brownness = np.clip((0.5 * a_centered + 0.9 * b_centered + 25.0) / 100.0, 0.0, 1.0).astype(np.float32)
        # Darkness: rust is often darker than clean metal
        darkness = np.clip((200.0 - lab[:, :, 0]) / 150.0, 0.0, 1.0).astype(np.float32)
    ...
    def _kmeans_pixel_classification(
        ...
        """
        KMeans on metal pixels only (using pass #1 mask to filter background).
        
        Robust fallback mechanism:
        - If SAM3 evidence is strong: uses SAM3 + color features
        - If SAM3 evidence is weak/absent: falls back to color-based features only
        """
        ...
        # Use metal mask to filter background pixels
        if metal_mask is not None:
            valid_mask = (metal_mask > 0).astype(bool)
        else:
        ...
        # Extract only valid (metal) pixels for KMeans
        X_raw_all = feature_tensor.reshape(-1, fdim).astype(np.float32)
        valid_indices = np.where(valid_mask.ravel())[0]
        X_raw = X_raw_all[valid_indices]
        ...
        # Assign all valid (metal) pixels to nearest center
        d2 = ((X[:, None, :] - centers[None, :, :]) ** 2).sum(axis=2)
        labels_valid = np.argmin(d2, axis=1).astype(np.int16)
        
        # Map labels back to full image (only for valid pixels)
        labels_all = np.full(h * w, fill_value=-1, dtype=np.int16)
        labels_all[valid_indices] = labels_valid

        # Prompt-guided rust cluster selection (uses pass2 evidence only, only on valid pixels)
        idx = {name: i for i, name in enumerate(feature_names)}
        cluster_scores: Dict[int, float] = {}
        
        # Check if SAM3 evidence is meaningful (max > threshold)
        sam_rust_strength = float(X_raw[:, idx["sam2_rust_evidence"]].max())
        sam_clean_strength = float(X_raw[:, idx["sam2_clean_evidence"]].max())
        sam_evidence_weak = (sam_rust_strength < 0.1) and (sam_clean_strength < 0.1)
        ...
        for c in range(K):
            ...
            if sam_evidence_weak:
                # Fallback: rely heavily on color when SAM3 evidence is weak
                score = (
                    1.5 * float(meanv[idx["redness"]])
                    + 1.3 * float(meanv[idx["brownness"]])
                    + 0.8 * float(meanv[idx["darkness"]])
                    + 0.4 * float(meanv[idx["texture_norm"]])
                    + 0.3 * float(meanv[idx["chroma_norm"]])
                    + 0.2 * float(meanv[idx["S_norm"]])
                    - 0.3 * float(meanv[idx["V_norm"]])
                    - 0.2 * float(meanv[idx["L_norm"]])
                )
            else:
                # Normal: use SAM3 evidence + color features
                score = (
                    2.8 * float(meanv[idx["sam2_rust_evidence"]])
                    - 2.2 * float(meanv[idx["sam2_clean_evidence"]])
                    + 0.6 * float(meanv[idx["redness"]])
                    + 0.55 * float(meanv[idx["brownness"]])
                    + 0.35 * float(meanv[idx["darkness"]])
                    + 0.20 * float(meanv[idx["texture_norm"]])
                    + 0.12 * float(meanv[idx["S_norm"]])
                    - 0.08 * float(meanv[idx["V_norm"]])
                )
        ...
        rust_pred_valid = (labels_valid == rust_cluster_id).astype(np.uint8)

        # Light refinement (pass2 evidence + color only) - only on valid pixels
        rust_i = idx["sam2_rust_evidence"]
        clean_i = idx["sam2_clean_evidence"]
        ...
        darkness_i = idx["darkness"]
        
        if sam_evidence_weak:
            # When SAM3 evidence is weak, use more lenient color-based support
            support = (
                (X_raw[:, redness_i] > 0.30)
                | (X_raw[:, brown_i] > 0.28)
                | (X_raw[:, darkness_i] > 0.35)
                | ((X_raw[:, redness_i] > 0.22) & (X_raw[:, brown_i] > 0.22))
                | ((X_raw[:, darkness_i] > 0.25) & (X_raw[:, brown_i] > 0.20))
            )
        else:
            # Normal: use SAM3 evidence + color features
            support = (
                ...
                | (X_raw[:, delta_i] > -0.02)
                | ((X_raw[:, redness_i] > 0.35) & (X_raw[:, brown_i] > 0.30))
                | ((X_raw[:, darkness_i] > 0.30) & (X_raw[:, brown_i] > 0.25))
            )
        rust_pred_valid = (rust_pred_valid & support.astype(np.uint8)).astype(np.uint8)
        
        # Map rust predictions back to full image
        rust_pred_all = np.zeros(h * w, dtype=np.uint8)
        rust_pred_all[valid_indices] = rust_pred_valid

        if self.ensure_one_positive and int(rust_pred_all.sum()) == 0 and n_valid > 0:
            rank = (
                2.0 * X_raw[:, rust_i]
                - 1.5 * X_raw[:, clean_i]
                + 0.6 * X_raw[:, redness_i]
                + 0.5 * X_raw[:, brown_i]
                + 0.4 * X_raw[:, darkness_i]
            )
            best_rust_in_valid = np.argmax(rank)
            best_idx_global = valid_indices[best_rust_in_valid]
            rust_pred_all[:] = 0
            rust_pred_all[best_idx_global] = 1

        cluster_map = labels_all.reshape(h, w).astype(np.int16)
        rust_mask = rust_pred_all.reshape(h, w).astype(np.uint8)
    ...
    def _perform_pixel_kmeans_analysis(self, crop: np.ndarray, metal_mask_crop: Optional[np.ndarray] = None) -> Dict:
        """
        PASS #2 on the ROI crop:
          - clean/rust SAM3 evidence maps
          - per-pixel feature vectors
          - KMeans on metal pixels only (using mask from pass #1)
        """
        h, w = crop.shape[:2]
        ...
        # Diagnostic: check if SAM3 detected anything
        if rust_out["num_detections"] == 0:
            self._log("  ⚠️  WARNING: SAM3 detected NO rust instances. Falling back to color-based detection.")
            self._log(f"     Try: --roi_pad 100 or --prompt_rust 'rust' or --prompt_rust 'corrosion'")
        if clean_out["num_detections"] == 0:
            self._log("  ⚠️  WARNING: SAM3 detected NO clean metal instances.")
        ...
        # KMeans on metal pixels only (using pass #1 metal mask)
        t0 = time.time()
        km = self._kmeans_pixel_classification(feature_tensor, feature_names, metal_mask_crop)
    ...
    def analyze(self, image_path: str, interactive: bool = True) -> Dict:
        ...
        # 1) PASS #1: SAM3 -> best metal box + mask for filtering background
        t0 = time.time()
        if interactive and self.sam3_predictor is not None:
            self._log(f"SAM3 pass #1: selecting MAX-score box for prompt={self.prompt_metal!r} + metal mask")
            metal_box_full, metal_box_score, metal_mask_full = self.get_best_metal_box(image_path, original)
        else:
            h, w = original.shape[:2]
            metal_box_full = (0, 0, w, h)
            metal_box_score = 0.0
            metal_mask_full = None
        ...
        # Also crop the metal mask if available
        metal_mask_crop = None
        if metal_mask_full is not None:
            metal_mask_crop = metal_mask_full[y1p:y2p, x1p:x2p].copy()
        
        timings["crop"] = time.time() - t0

        self._log("PASS #1 complete: ROI box + metal mask extracted for background filtering.")

        # 3) PASS #2 + per-pixel KMeans on metal pixels only (filtered by pass #1 mask)
        t0 = time.time()
        self._log("Running PASS #2 (clean/rust prompts) + per-pixel KMeans on metal pixels only...")
    ...
    def visualize_detection(self, results: Dict, save_path: str | None = None) -> plt.Figure:
        ...
        fig.suptitle(
            "Rust Detection Stages (SAM3 pass1 box+mask -> SAM3 pass2 clean/rust on ROI -> per-pixel KMeans on metal only)\n"
        ...
        titles = [
            "1) Input image",
            "2) Input + best metal box (SAM3 pass #1, crop + mask)",
            "3) Cropped ROI (from max-score box)",
            "4) SAM3 pass #2 evidence on ROI (clean=green, rust=red)",
            "5) KMeans on metal pixels only + final rust mask",
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Rust Detection: SAM3 pass1 metal ROI box+mask -> SAM3 pass2 clean/rust evidence on ROI -> per-pixel KMeans on metal pixels only"
    )
    ...
    p.add_argument("--roi_pad", type=int, default=50, help="Padding (px) around metal box before cropping (gives SAM3 more context in pass #2).")
    ...
    p.add_argument("--prompt_rust", type=str, default="rust corrosion", help="SAM3 prompt for pass #2 rust evidence. Try: 'rust', 'corrosion', 'rust corrosion', 'oxidation'.")
...
def main():
    ...
    if bool(args.save_features):
        ...
        print("CONFIRMED: pass #1 extracts ROI box + metal mask; pass #2 classifies metal pixels only.")
