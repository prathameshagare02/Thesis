...
class FastRustDetector:
    ...
    def _compute_dynamic_gates(self, features: np.ndarray) -> Dict[str, float]:
        """
        UPDATED: more conservative gates to reduce brushed-metal false positives.
        - Push "warm/red/orange" gates upward (higher percentiles).
        - Require more saturation (raise s_low/mid/hi).
        - Treat "dark" less aggressively (raise v_dark percentile a bit).
        - Slightly raise texture gates.
        """
        ...
        # dyn = {
        #     # Stricter "warm" gate: require stronger redness (reduces yellow/gold glare hits)
        #     "a_warm": self._robust_percentile(a_vals, 65, 145.0),
        #     "b_warm": self._robust_percentile(b_vals, 60, 140.0),

        #     # Stricter "red" gate: reduces false positives on yellow-ish metal reflections
        #     "a_red": self._robust_percentile(a_vals, 85, 160.0),
        #     "a_red_hi": self._robust_percentile(a_vals, 93, 175.0),

        #     # Keep orange/yellow fairly strict (rust still passes, blue/steel fails)
        #     "b_orange": self._robust_percentile(b_vals, 82, 155.0),
        #     "b_orange_hi": self._robust_percentile(b_vals, 92, 170.0),

        #     # Mark highlights earlier so specular/golden shine can be suppressed downstream
        #     "v_dark": self._robust_percentile(v_vals, 30, 175.0),
        #     "v_very_dark": self._robust_percentile(v_vals, 15, 80.0),
        #     "v_hi": self._robust_percentile(v_vals, 95, 235.0),

        #     # Slightly higher saturation floors help remove pale/washed reflections
        #     "s_low": self._robust_percentile(s_vals, 60, 60.0),
        #     "s_mid": self._robust_percentile(s_vals, 75, 85.0),
        #     "s_hi": self._robust_percentile(s_vals, 90, 110.0),

        #     "rough_hi": self._robust_percentile(rough_vals, 80, 20.0),
        #     "ent_hi": self._robust_percentile(ent_vals, 80, 12.0),
        # }
        dyn = {
            # Make "warm" stricter (neutral is ~128 in OpenCV LAB)
            "a_warm": self._robust_percentile(a_vals, 60, 140.0),
            "b_warm": self._robust_percentile(b_vals, 60, 140.0),

            # Make "red" stricter
            "a_red": self._robust_percentile(a_vals, 82, 155.0),
            "a_red_hi": self._robust_percentile(a_vals, 92, 170.0),

            # Make "orange/yellow" stricter
            "b_orange": self._robust_percentile(b_vals, 82, 155.0),
            "b_orange_hi": self._robust_percentile(b_vals, 92, 170.0),

            # Keep value thresholds mostly as-is (don’t rely on darkness to decide rust)
            "v_dark": self._robust_percentile(v_vals, 30, 175.0),
            "v_very_dark": self._robust_percentile(v_vals, 15, 80.0),
            "v_hi": self._robust_percentile(v_vals, 98, 245.0),

            # Raise saturation floors a bit to suppress low-sat bluish/steel regions
            "s_low": self._robust_percentile(s_vals, 55, 55.0),
            "s_mid": self._robust_percentile(s_vals, 70, 75.0),
            "s_hi": self._robust_percentile(s_vals, 88, 100.0),

            "rough_hi": self._robust_percentile(rough_vals, 80, 20.0),
            "ent_hi": self._robust_percentile(ent_vals, 80, 12.0),
        }
        # --- UPDATED percentile choices + defaults ---

      
        # dyn = {
        #     "a_warm": self._robust_percentile(a_vals, 45, 132.0),
        #     "b_warm": self._robust_percentile(b_vals, 45, 130.0),

        #     "a_red": self._robust_percentile(a_vals, 75, 150.0),
        #     "a_red_hi": self._robust_percentile(a_vals, 85, 160.0),

        #     "b_orange": self._robust_percentile(b_vals, 75, 150.0),
        #     "b_orange_hi": self._robust_percentile(b_vals, 85, 160.0),

        #     "v_dark": self._robust_percentile(v_vals, 30, 175.0),
        #     "v_very_dark": self._robust_percentile(v_vals, 15, 80.0),
        #     "v_hi": self._robust_percentile(v_vals, 98, 245.0),

        #     "s_low": self._robust_percentile(s_vals, 40, 45.0),
        #     "s_mid": self._robust_percentile(s_vals, 60, 60.0),
        #     "s_hi": self._robust_percentile(s_vals, 85, 90.0),

        #     "rough_hi": self._robust_percentile(rough_vals, 80, 20.0),
        #     "ent_hi": self._robust_percentile(ent_vals, 80, 12.0),
        # }

        # --- UPDATED clamps (keep gates from becoming too permissive) ---
        dyn["a_warm"] = float(np.clip(dyn["a_warm"], 118.0, 165.0))
        dyn["b_warm"] = float(np.clip(dyn["b_warm"], 118.0, 165.0))

        dyn["a_red"] = float(np.clip(dyn["a_red"], dyn["a_warm"] + 4.0, 185.0))
        dyn["a_red_hi"] = float(np.clip(dyn["a_red_hi"], dyn["a_red"] + 2.0, 195.0))

        dyn["b_orange"] = float(np.clip(dyn["b_orange"], dyn["b_warm"] + 4.0, 190.0))
        dyn["b_orange_hi"] = float(np.clip(dyn["b_orange_hi"], dyn["b_orange"] + 2.0, 205.0))

        dyn["v_very_dark"] = float(np.clip(dyn["v_very_dark"], 35.0, 130.0))
        dyn["v_dark"] = float(np.clip(dyn["v_dark"], dyn["v_very_dark"] + 15.0, 225.0))
        dyn["v_hi"] = float(np.clip(dyn["v_hi"], 220.0, 255.0))

        dyn["s_low"] = float(np.clip(dyn["s_low"], 15.0, 120.0))
        dyn["s_mid"] = float(np.clip(dyn["s_mid"], dyn["s_low"] + 5.0, 160.0))
        dyn["s_hi"] = float(np.clip(dyn["s_hi"], dyn["s_mid"] + 5.0, 220.0))

        dyn["rough_hi"] = float(np.clip(dyn["rough_hi"], 6.0, 55.0))
        dyn["ent_hi"] = float(np.clip(dyn["ent_hi"], 4.0, 35.0))
    ...
    def _rust_score_for_feature(self, fv: np.ndarray, dyn: Optional[Dict[str, float]] = None) -> float:
        ...
        # UPDATED: narrower rust-hue gate (OpenCV H is 0..179)
        # Rust/orange/brown tends to cluster closer to red/orange than broad "<55".
        ...
        if mean_s > dyn["s_hi"]:
            ...
            # keep only a tiny contribution for dark low-s regions
    ...
    def analyze(self, image_path: str, interactive: bool = True) -> Dict:
        ...
        # TERMINATE if score is None or 0
